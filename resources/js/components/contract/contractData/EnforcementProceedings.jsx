...
const EnforcementProceedings = ({ executiveDocId, setShow }) => {
    ...
    useEffect(() => {
        api.get(getContractPath('enforcement-proceedings/get-list-by-executive-doc/' + executiveDocId))
            ...
            if (data && Array.isArray(data)) {
                ...
                if (data.length !== 0)
                    setActiveEnforcementProceeding(data[data.length - 1]);
            }
        })
    ...
    return (<CustomModal customStyles={{ width: 500 }} setShow={setShow}>
            {loading ? <Loading /> :
            <CustomStepper dataArray={enforcementProceedings} setDataArray={setEnforcementProceedings} setActiveData={setActiveEnforcementProceeding} setDeleteIds={setDeleteIds}>
                    ...
                    <CreateBailiff setShow={setShowCreateBailiff} setNewItem={setBailiff}/>}
                        <div className="small-inputs-box">
                            <EasyInput ref={beginDateRef} autoFocus defaultValue={activeEnforcementProceeding.begin_date} label='дата начала исп. производства' className={classes.smallInput} type='date' name='begin_date' required/>
                            <EasyInput ref={endDateRef} autoFocus label='дата окончания исп. производства' className={classes.smallInput} type='date' name='end_date'/>
                        </div>
                        <div className="small-inputs-box">
                            <EasyInput ref={numberRef} label='Номер исп. производства' className={classes.smallInput} name='number'/>
                        </div>
                        ...
                        <div className={styles.contentBlock}>
                            <EasyInput ref={mainRef} className={styles.smallInput} size={'small'} name='main' variant='standard' pattern='float' required label='осн. долг'/>
                            <EasyInput ref={percentsRef} className={styles.smallInput} size={'small'} name='percents' variant='standard' pattern='float' required label='Проценты'/>
                        </div>
                        <div className={styles.contentBlock}>
                            <EasyInput ref={penaltiesRef} className={styles.smallInput} size={'small'} name='penalties' variant='standard' pattern='float' required label='Неустойка'/>
                            <EasyInput ref={feeRef} className={styles.smallInput} size={'small'} name='fee' variant='standard' pattern='float' required label='Госпошлина'/>
                        </div>
