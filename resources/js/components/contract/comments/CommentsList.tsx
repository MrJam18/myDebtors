...
const onClickFile = async (ev: MouseEvent)=> {
    ev.stopPropagation();
    const data = (ev.currentTarget as HTMLButtonElement).getAttribute('data');
    try {
        await saveFile(data);
    } catch (e) {
...
const CommentsList = ({}) => {
    ...
    useEffect(() => {
        if(changedComment) {
            if (changedComment.user_id !== userId) Alert.setError('Ошибка при изменении комментария', new Error("Вы не являетесь владельцем комментария и не можете изменить его"));
            else showChangeComment.setTrue();
        }
    }, [changedComment]);
    return (
