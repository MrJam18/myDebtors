...
const CustomStepper = ({ dataArray, setDataArray, setActiveData, defaultActiveStep = null, children, leftTopButtons = null, setDeleteIds, onChangeStep = null }) => {
    const [activeStep, setActiveStep] = useState(defaultActiveStep !== null && defaultActiveStep !== void 0 ? defaultActiveStep : dataArray.length - 1);
    ...
    return (<>
            ...
            {children}
        <MobileStepper variant="dots" steps={dataArray.length} position="static" activeStep={activeStep} sx={{ maxWidth: 1000, flexGrow: 1 }} nextButton={<Button size="small" type='button' onClick={handleNext} className={styles.bottomButton} disabled={activeStep === dataArray.length - 1}>
                    след.
