...
const CourtClaimChanger = ({ setShow, courtClaimId = null, update }) => {
    ...
    const onSubmit = (ev) => {
        ...
        if (courtClaimId)
            dispatcher.addData('courtClaimId', courtClaimId);
            ...
            <form onSubmit={onSubmit} ref={form.ref}>
                    ...
                    <div className={styles.flexBetween + ' margin_0'}>
                        <div className={styles.smallBlock}>
                            <CustomInput type={'date'} defaultValue={courtClaim.count_date} className={styles.fullWidthBlock} label={'Дата расчета иска'} name={'count_date'}/>
                            <ServerSelect required defaultId={courtClaim.statusId} customClassName={styles.fullWidthBlock} name={'status_id'} label={'Cтатус'} serverAddress={'court-claims/get-statuses'}/>
                            <div className={styles.fullWidthBlock}>
                                Статус обновлен: {courtClaim.status_date}
                            </div>
                        </div>
                        <div className={styles.smallBlock}>
                            <EasyCheckBox defaultValue={courtClaim.is_ignored_payments} label={'Игнорировать платежи при ограничении процентов'} name={'is_ignored_payments'}/>
                    ...
                    <div className={styles.flexBetween}>
                        ...
                        <EasyInput defaultValue={courtClaim.percents} required className={styles.smallInput} name={'percents'} pattern={'float'} label={'проценты'}/>
                    </div>
                    <div className={styles.flexBetween}>
                        <EasyInput required defaultValue={courtClaim.penalties} className={styles.smallInput} name={'penalties'} pattern={'float'} label={'неустойка'}/>
