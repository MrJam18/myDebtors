...
export const addLinksForSaveFile = (rows, onClick) => {
    ...
    rows.forEach((el) => {
        ...
        if (regExp.test(el.result))
            row.result = <button onClick={onClick} data-id={el.id} data-object={el.actionObject.name} data-path={el.result} className={'antibutton' + ' ' + styles.actions__downloadButton}>
