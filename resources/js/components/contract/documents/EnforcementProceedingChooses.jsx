...
const EnforcementProceedingChooses = ({ selectedDoc, setError, setLoading }) => {
    ...
    const [enforcementProceeding, setEnforcementProceeding] = useState({
        name: 'Отсутствует'
    });
    ...
    const onSubmit = (ev) => {
        ev.preventDefault();
        if (!enforcementProceeding.id)
            setError('Сначала создайте исполнительное производство');
        else {
    ...
    return (<>
            ...
            <form id='submitSelectDocument' className={styles.documents__executiveChoosesMain} onSubmit={onSubmit}>
                    ...
                    <div className={styles.executiveChooses__ExecutiveDocName} onClick={showEnforcementProceedings.setTrue}>{enforcementProceeding.name}</div>
                    <h4 className={styles.smallHeader}>
                        Представитель
                    </h4>
