...
export const changeContract = (data, contractId) => async (dispatch) => {
    try {
        await api.post('contracts/changeContract', Object.assign(Object.assign({}, data), { contractId }));
...
export const createIPInitDoc = (contractId, agentId) => async (dispatch, getState) => {
    if (!getState().contracts.executiveDoc.id)
        throw new Error('Укажите данные исполнительного документа!');
...
export const setExecutiveDoc = (formData, court, bailiff, typeId, contractId, executiveDocId) => async (dispatch) => {
    ...
    const sendingExecutiveDoc = Object.assign(Object.assign({}, formData), { courtId: court.id, bailiffId: bailiff.id, typeId,
        contractId, id: executiveDocId || null });
    console.log(sendingExecutiveDoc);
    const { data } = await api.post('executiveDocs/setExecutiveDoc', sendingExecutiveDoc);
    const executiveDocForStore = Object.assign(Object.assign({}, formData), { court,
        bailiff,
        typeId, id: data.id });
