...
// @ts-ignore
const AddButton = () => <FileUploadIcon sx={{ color: 'black', width: '25px', height: '25px' }} className={styles.documents__uploadIcon + ' ' + styles.documents__icon}/>;
const loadObjects = [
...
const ExcelLoader = ({ setShow, update }) => {
    ...
    const [loadEntity, setLoadEntity] = useState(null);
    const [file, setFile] = useState(null);
    const form = useForm({ setShow, buttonText: 'Загрузить', noDispatcher: true });
    const onLoadTemplate = async () => {
        const object = loadObjects.find((type) => type.id === loadEntity);
    ...
    return (<CustomModal customStyles={{ width: '400px' }} header={'Выгрузка Excel'} setShow={setShow}>
