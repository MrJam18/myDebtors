...
const CourtClaimChanger = ({ setShow, update }) => {
    ...
    const updateInputs = (data) => {
        ...
        if (formRef.current && data) {
            ...
            const updateElements = createUpdateElementsFunc(data, elements);
            ...
            updateElements('fee');
            setTypeId((_a = data.type_id) !== null && _a !== void 0 ? _a : 0);
            setStatusId((_b = data.status_id) !== null && _b !== void 0 ? _b : 0);
            setAgent(data.agent);
            setCourt(data.court);
        }
    };
    ...
    useEffect(() => {
        ...
        api.get('agents/get-default')
            .then(({ data }) => {
            if (data)
                defaultData.agent = data;
        })
            .catch((e) => Alert.setError('Ошибка при получении агента по умолчанию', e));
    }, []);
    return (<CustomModal setShow={setShow} header={'Изменение судебного иска'} customStyles={{ width: 500 }}>
            {showAddCourt.Comp()}
            {loading ? <Loading /> : <>
                <CustomFormStepper dataArray={claimsArray} setDataArray={setClaimsArray} setActiveData={setActiveClaim} setDeleteIds={setDeleteIds} getUpdatedData={getUpdatedData} onSubmit={onSubmit} loading={buttonLoading} onChangeStep={updateInputs} defaultData={defaultData} onDeleteAll={onDeleteAll} ref={formRef}>
                    ...
                    <div className={styles.flexBetween + ' margin_0'}>
                        <div className={styles.smallBlock}>
                            <CustomInput type={'date'} className={styles.fullWidthBlock} label={'Дата расчета иска'} name={'count_date'}/>
                ...
                <div className={"flex"} onClick={onDownloadDocument} style={{ alignItems: 'center', cursor: 'pointer' }}>
                    ...
                    <EasySearch required label={'Представитель по делу'} serverAddress={'agents/search-list'} setValue={setAgent} value={agent}/>
                    ...
                    <div className={styles.flexBetween}>
                        ...
                        <EasyInput required className={styles.smallInput} name={'percents'} pattern={'float'} label={'проценты'}/>
                    </div>
                    <div className={styles.flexBetween}>
                        <EasyInput required className={styles.smallInput} name={'penalties'} pattern={'float'} label={'неустойка'}/>
