...
export class RegistrationDispatcher extends Dispatcher {
    async _handler(dispatcherData) {
        if (dispatcherData.formData.password.length < 8)
            throw new Error("Минимальная длина пароля должна быть не менее 8 символов");
        if (dispatcherData.formData.password !== dispatcherData.formData.passwordConfirm)
            throw new Error('Пароли не совпадают');
        if (!dispatcherData.group_id && !dispatcherData.formData.groupName)
            throw new Error('Укажите группу для вступления');
